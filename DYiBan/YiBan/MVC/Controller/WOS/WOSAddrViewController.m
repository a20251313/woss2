...
//        arrayAddrList = [[NSMutableArray alloc]init];
        delIndex = 0;
        MagicRequest *request = [DYBHttpMethod wosKitchenInfo_addrList_userIndex:SHARED.userId page:@"0" count:@"100" sAlert:YES receive:self];
        [request setTag:3];
...
-(void)delSueese:(NSString *)strIndex{
    ...
    for (int i = 0; i< arrayAddrList.count ; i++) {
        ...
        if ([[NSString stringWithFormat:@"%@",strIndex ] isEqualToString:[[dict objectForKey:@"addrIndex"]stringValue]]) {
